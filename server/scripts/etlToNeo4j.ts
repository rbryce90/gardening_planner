...
async function etl() {
    ...
    const plants = db.prepare(
        "SELECT id, name, category, growth_form, edible_part, family FROM plants"
    ).all() as any[];
    ...
    const companions = db.prepare(
        "SELECT plant_id, companion_id FROM companions"
    ).all() as any[];
    ...
    const antagonists = db.prepare(
        "SELECT plant_id, antagonist_id FROM antagonists"
    ).all() as any[];
