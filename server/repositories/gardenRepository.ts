...
export class GardenRepository {
    ...
    async createGarden(userId: number, name: string, rows: number, cols: number): Promise<Garden> {
        ...
        const result = db.prepare(
            "INSERT INTO gardens (user_id, name, rows, cols) VALUES (?, ?, ?, ?)"
        ).run(userId, name, rows, cols);
        const id = Number(result.lastInsertRowid);
        const row = db.prepare(
            "SELECT id, user_id, name, rows, cols, created_at FROM gardens WHERE id = ?"
        ).get(id) as any;
        return {
            id: row.id,
            userId: row.user_id,
    ...
    async clearCell(gardenId: number, row: number, col: number): Promise<boolean> {
        const db = getDatabase();
        const result = db.prepare(
            "DELETE FROM garden_cells WHERE garden_id = ? AND row = ? AND col = ?"
