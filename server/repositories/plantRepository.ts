...
export class PlantRepository {
  ...
  async createPlantType(plantId: string, plantType: any): Promise<any> {
    ...
    const id = v4();
    db.prepare(
      "INSERT INTO plant_types (id, plant_id, name, description, planting_notes) VALUES (?, ?, ?, ?, ?)",
    ).run(id, plantId, plantType.name, plantType.description, plantType.planting_notes);
    return { ...plantType, id };
  }
  ...
  async addCompanion(plant_id: string, companion_id: string): Promise<void> {
    const db = getDatabase();
    db.prepare("INSERT INTO companions (plant_id, companion_id) VALUES (?, ?)").run(
      plant_id,
      companion_id,
    );
  }
  ...
  async deletePlantType(plantId: string, typeId: string): Promise<boolean> {
    const db = getDatabase();
    const result = db
      .prepare("DELETE FROM plant_types WHERE id = ? AND plant_id = ?")
      .run(typeId, plantId);
    return result.changes > 0;
  }

  async deleteCompanion(plantId: string, companionId: string): Promise<boolean> {
    const db = getDatabase();
    const result = db
      .prepare(
        "DELETE FROM companions WHERE (plant_id = ? AND companion_id = ?) OR (plant_id = ? AND companion_id = ?)",
      )
      .run(plantId, companionId, companionId, plantId);
    return result.changes > 0;
  }

  async deleteAntagonist(plantId: string, antagonistId: string): Promise<boolean> {
    const db = getDatabase();
    const result = db
      .prepare(
        "DELETE FROM antagonists WHERE (plant_id = ? AND antagonist_id = ?) OR (plant_id = ? AND antagonist_id = ?)",
      )
      .run(plantId, antagonistId, antagonistId, plantId);
    return result.changes > 0;
  }
  ...
  async createPlantingSeason(plantTypeId: string, plantingSeason: any): Promise<any> {
    ...
    const id = v4();
    db.prepare(
      "INSERT INTO planting_seasons (id, plant_type_id, zone_id, start_month, end_month, method, notes) VALUES (?, ?, ?, ?, ?, ?, ?)",
    ).run(
      id,
      plantTypeId,
